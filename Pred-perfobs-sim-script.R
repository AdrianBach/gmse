...
trajVSpred_replicate <- function(ts = 10, rep = 10, freq = 1,
                                bdgt = 1000, trgt = 2000, stkh = 4, obstype = 3,
                                popinit = 1000, tf = 12, cons = 0.5, surv = 4.75, repr = 5, 
                                ldim = 100,
                                out_file = "predVSregular-results.csv") {
  ...
  results <- array(data=NA, dim = c(rep, length(columns)), dimnames = list(NULL,columns))
  ...
  for (m in prd) {
    ...
    for (k in 1:rep) {
      ...
      # Pick up values for simulation results and store them in results
      
      # Replicate number
      results[k,2] <- k
      
      # Prediction?
      results[k,1] <- as.numeric(m)
      
      # Has extinction occured? (yes = 1, no = 0)
      results[k,3] <- ifelse(final_ts < dim(sim$paras)[1], 1, 0)
      
      # Resource actual pop deviation from target at ts before last
      results[k,4] <- dim(sim$resource[[final_ts-1]])[1]/sim$action[[1]][1,5,1] - 1
     
      # Users total final yield at ts before last
      results[k,6] <- sum(sim$agents[[final_ts-1]][,16])
      
      # Maximum difference between Users yield at ts before last
      results[k,8] <- (max(sim$agents[[final_ts-1]][,16]) - min(sim$agents[[final_ts-1]][-1,16]))/max(sim$agents[[final_ts-1]][,16])
      
      # final ts
      results[k,11] <- final_ts
      ...
      if (results[k,3] != 0) {
        ...
        results[k,10] <- ssd + abs(-100)
      }
      
      # If extinction did not occured
      else {    
          # Resource actual pop deviation from target at last ts
          results[k,5] <- dim(sim$resource[[final_ts]])[1]/sim$action[[1]][1,5,1] - 1
          
          # Users total final yield at last ts
          results[k,7] <- sum(sim$agents[[final_ts]][,16])
          
          # Maximum difference between Users yield at last ts
          results[k,9] <- (max(sim$agents[[final_ts]][,16]) - min(sim$agents[[final_ts]][-1,16]))/max(sim$agents[[final_ts]][,16])
